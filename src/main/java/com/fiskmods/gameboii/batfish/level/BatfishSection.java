...
public class BatfishSection
{
    ...
    {
        int x = Engine.getWidth() / ScreenIngame.SCALE / 2 - 10;
        int y = index * HEIGHT;
        ...
        if (index > 0)
        {
            int w = Engine.getWidth() / ScreenIngame.SCALE / 2;
            int padding = 20;
            int x1 = w - 15;
            int i = 13;
            ...
            int left = -x1;
            int right = x1;
            int mid = GameboiiMath.getRandomIntegerInRange(rand, left + padding, right - padding);
    ...
    private int getSafePosX(Random rand, int min, int max, int mid, int midR)
    {
        ...
        {
            x = GameboiiMath.getRandomIntegerInRange(rand, min, max);
        }
